...
export async function seed(count = seedCountFromEnvironment()): Promise<void> {
  ...
  await withTransaction(async (client) => {
    await client.query("TRUNCATE user_hobbies, users, hobbies RESTART IDENTITY CASCADE");
    ...
    for (let start = 0; start < data.users.length; start += batchSize) {
      ...
      const placeholders = users.map((user, index) => {
        ...
        values.push(user.avatar, user.firstName, user.lastName, user.age, user.nationality);
    ...
    const userIds = await client.query<{ id: string }>("SELECT id FROM users ORDER BY id");
    const joinRows = data.users.flatMap((user, index) => user.hobbies.map((hobby) => [userIds.rows[index].id, hobby]));
    for (let start = 0; start < joinRows.length; start += batchSize) {
      ...
      if (placeholders.length > 0) {
        await client.query(`INSERT INTO user_hobbies (user_id, hobby_value) VALUES ${placeholders.join(", ")}`, values);
      }
    }
  });
}
...
function seedCountFromEnvironment(): number {
  const value = process.env.SEED_COUNT;
