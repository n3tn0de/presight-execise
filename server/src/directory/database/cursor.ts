...
function signature(query: DirectoryQuery): string {
...
export function encodeCursor(
  input: DirectoryCursor & { query: DirectoryQuery },
): string {
  return encode(
    JSON.stringify({
      signature: signature(input.query),
      sortBy: input.query.sortBy,
      sortDir: input.query.sortDir,
...
export function decodeCursor(
  ...
  let payload: unknown;
  try {
    payload = JSON.parse(decode(cursor));
  } catch {
  ...
  if (
    ...
    keys.some((key, index) => key !== payloadKeys.slice().sort()[index])
  )
    throw new CursorError();
  ...
  if (
    ...
    !sortFields.includes(candidate.sortBy as string) ||
    !sortDirections.includes(candidate.sortDir as string) ||
    candidate.signature !== signature(query) ||
    candidate.sortBy !== query.sortBy ||
    candidate.sortDir !== query.sortDir ||
  ...
  return {
    sortValue: candidate.sortValue as DirectoryCursor["sortValue"],
    id: candidate.id,
  };
}
