import type { DirectoryQuery, SortDirection, SortField } from "../types/api";
...
function parseLimit(value: string | null): number {
  if (value === null || !/^\d+$/.test(value))
    return DEFAULT_DIRECTORY_QUERY.limit;
  return Math.min(
    MAX_DIRECTORY_LIMIT,
    Math.max(MIN_DIRECTORY_LIMIT, Number(value)),
  );
}
...
function normalizeLimit(value: number): number {
  ...
  return Math.min(
    MAX_DIRECTORY_LIMIT,
    Math.max(MIN_DIRECTORY_LIMIT, Math.trunc(value)),
  );
}
...
export function serializeDirectoryQuery(query: DirectoryQuery): string {
  ...
  if (sortDir !== DEFAULT_DIRECTORY_QUERY.sortDir)
    params.set("sortDir", sortDir);
  if (limit !== DEFAULT_DIRECTORY_QUERY.limit)
